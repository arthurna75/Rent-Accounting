...
const ENTRY_TYPES: JournalEntryType[] = [
  '비용지출', '카드지출', '카드결제', '임대수익', '보증금수령', '보증금반환', '일반',
]
...
const ENTRY_FILTER: Record<string, { debit: string[]; credit: string[] }> = {
  ...
  '카드지출':  { debit: ['비용'],  credit: ['부채'] },
  '카드결제':  { debit: ['부채'],  credit: ['자산'] },
...
const DEFAULT_CODES: Record<string, { debit: string; credit: string }> = {
  ...
  '카드지출':  { debit: '',    credit: '322' }, // 대변: 미지급금 (카드대금 부채)
  '카드결제':  { debit: '322', credit: '102' }, // 차변: 미지급금, 대변: 보통예금
