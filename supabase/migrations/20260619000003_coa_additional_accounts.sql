-- ============================================================
-- 계정과목 추가: 지급수수료·제세공과금·소모품비·자본금(기존 누락분)
-- ============================================================

-- 1. 기존 조직에 누락 계정 삽입 (이미 있으면 무시)
...
CROSS JOIN (VALUES
  ('410', '자본금',    '자본', '자본금',    '대변'),
  ('643', '제세공과금', '비용', '영업비용',  '차변'),
  ('662', '지급수수료', '비용', '일반관리비', '차변'),
  ('671', '소모품비',   '비용', '일반관리비', '차변')
) AS v(code, name, account_type, account_subtype, normal_balance)
ON CONFLICT (organization_id, code) DO NOTHING;

-- 2. 신규 조직 seed 함수에 동일 계정 반영 (CREATE OR REPLACE)
